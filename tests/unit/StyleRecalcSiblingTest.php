...
{
    // C2.9：由 ThemeProvider 注册表迁至 StyleEngine（CSS 串 → StyleSheetContents
    // → SelectorChecker 全 AST 匹配）——与 gen 生产形态同源。
    StyleEngine::reset();
...
test('后代选择器跨中间元素匹配任意祖先（C2.5）', function () {
    ...
    StyleEngine::reset();
    StyleEngine::registerCss('.gc { width: 80px; } .gp .gc { width: 160px; }');
...
test('child > 严格仅直接父（不匹配祖父）', function () {
    ...
    StyleEngine::reset();
    StyleEngine::registerCss('.cc { width: 80px; } .cp > .cc { width: 160px; }');
    // 跨中间：cp > cmid > cc，> 不应命中
...
test('通用兄弟 ~ 跨中间兄弟匹配', function () {
    ...
    StyleEngine::reset();
    StyleEngine::registerCss('.gt { width: 80px; } .ga ~ .gt { width: 160px; }');
