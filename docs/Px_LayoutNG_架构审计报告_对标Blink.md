...
| 2026-07-28 | **case-050 text-emphasis 行高只读归因（推测回退零残留，前置件锁定）**：真值 px-79/117（.te-circle/.te-dot 盒）B h35 vs E h25 恒 +10@fs16，上传至 te-item（px-86 98 vs 88）。尝试在 layoutInlineRun 加 emphasis over 注解 0.5em（css-text-decor-3 §8.1/§8.4，intdiv(fs+1,2)=8@16，预算 25+8=33 vs B35 入 tol=2）——**零效果**。**根因定位**：`text-emphasis` 简写未在 CssMappings 注册展开（仅长手 text-emphasis-style/color/position，且 textEmphasisStyle 无任何消费者）→ `.te-circle{text-emphasis:circle #e94560}` 声明整个丢弃→ textEmphasisStyle 永不置值→ 行高增补无法触发。按纪律回退推测性 InlineAlgorithm 改动（零残留，git status 净）。**下批（完整件）**：① CssShorthandExpander 加 text-emphasis → style(+color)；② 恢复 layoutInlineRun 的 0.5em over 注解行高（已验算入 tol）——两件合一批端到端验证 |
| 2026-07-28 | **✅ text-emphasis 简写展开 + over 注解行高批（治本，@0365bacd）：276→202（-74）**：上批归因的两件前置件合一落地——① CssShorthandExpander 新增 expandTextEmphasis：`text-emphasis` → text-emphasis-style/color（css-text-decor-3 §8.3，fill/shape 归 style、#/rgb/命名色归 color；kebab-only 写入保 dispatch 安全，同 text-decoration 模式）；② layoutInlineRun：容器 textEmphasisStyle 非 none 时行高 +0.5em over 注解（§8.1 marks=0.5em、§8.4 leading 不足时行盒 over 侧增高，intdiv(fs+1,2)=8@16，文本下移让出注解带）。验证：case-050 **276→202（-74）**（te 盒 h25→33 入 tol=2、上传 te-item），全量 **1419→1345**，31/32 保持、零劣化。需 sfc 重编（简写展开编译期烘焙）。累计：4500→1345（**-70%**），20 批全治本 |
| 2026-07-28 | **case-039 va 行盒位移扩展只读归因（下批靠点，零入库）**：剩余 162 主体 = **85 条 d6 均匀 y 族**（px-63 va 行盒 E h51 vs B h57，子 span 因行盒短 6 而位高 6）。根因：LineBreaker computeLineHeight 仅用 item 原始 ascent/descent 累计行高，**未计入 vertical-align sub/super/text-top/text-bottom 位移量**——位移盒超出基线部分未扩展行盒 ascent/descent（对标 Blink NGInlineBoxState/NGLineHeightMetrics：每项 effective ascent=ascent+shiftUp、descent=descent+shiftDown）。va 位移量现在 InlineAlgorithm 放置侧 switch 计算（b8bb0e9b），LineBreaker 行高侧未消费。**下批方案**：抽共享 va 位移 helper（避免常量双写概念错乱）→ LineBreaker 累计 ascent/descent 时按 helper 扩展 + InlineAlgorithm 放置侧复用同 helper；属跨模块，需全量+门验证（L多个 va/inline 测例），单独成批 |
| 2026-07-28 | **case-039 va 行盒扩展归因精化（per-item 假设否定，回退零残留）**：实现共享 vaShiftExtent helper + LineBreaker 按 item.va 扩展 ascent/descent——**零效果**。深层原因：px-63 行的 va-super/va-sub 是 **内联盒（OPEN_TAG）** 而非 atomic 项；va 属性在盒上，盒内 marks（inline-block h25）为 baseline。LineBreaker 中 OPEN_TAG 为标记项（ascent=0），item.va 检查对它无效；真正需要的是**盒 vShift 上下文传播到被包含的 atomic marks**（盒位移后其内容 marks 的 ascent/descent 贡献隨之上/下移）——这与 InlineAlgorithm 放置侧 boxStack vShift（b8bb0e9b）同构但发生在行高计算时。**修正后下批方案**：LineBreaker 需追踪 open box va 栈（同 NGInlineBoxState 盒栈），对栈内 atomic 项的 ascent/descent 按累积 vShift 扩展——需在 LineBreaker 引入盒栈状态（比 per-item helper 复杂），vaShiftExtent helper 可复用作单源。本轮推测性 helper+LineBreaker 改动已全回退（git status 净） |
| 2026-07-28 | **⚠ 重大纠正：“环境级 GUI exe 阻塞”结论推翻（用户提示 .bat 可开 exe）**：前多轮将 AOT exe cpu=0 挂死归因为“锁屏/无人值守会话环境级阻塞、所有 GUI exe 不可用”——**误诊**。真因：诊断用的 PowerShell `Start-Process -WindowStyle Hidden` 隐藏窗口启动，无窗口站致 Skia/Win32 后端初始化阻塞（进程存活 CPU=0）。验证：用户指出 test_case 下 .bat（前台可见窗口）能正常开 exe；实测同一 exe 前台 `& .\css_test.exe --case=X --headless --dump-layout` **exit=0 且产出 engine_layout_aot.json**；css-test 管线本就用 `exec()` 前台同步调用（LayoutDumpStep），AOT dump 正常。**纠正后真实唯一卡点 = build 编译慢**（framework 改动后 exe 陈旧需重编），非 exe 不可运行。**行动**：先前“待用户在场才能跑 AOT”的前提作废；已启动 test_pipeline（AOT 模式）后台重编 exe→exec dump _aot.json→compare_php_aot 55/55（编译完成后出结）；记忆已纠正（禁用 -WindowStyle Hidden 启 GUI exe） |
| 2026-07-28 | **✅✅ AOT 验证收结：compare_php_aot 54/55 identical**（重编 exe @10:48 + 管线 exec dump 55 case _aot.json 全新）：**54 case CLI≡AOT 完全一致**，唯一 diff = case-050（geo=43, n 268 vs 268）。分叉性质核实：**全部是 .x 坐标**（php=490 vs aot=411）且集中在 ::before CJK 内容“生成内容”下游——**我的 text-emphasis emphExtra(y 位移) 在 CLI/AOT 完全一致**（整数确定性），分叉纯粻 CLI fallback measure（strlen 近似）vs AOT 真实 sk_measure_text_width 字体 metrics 在 CJK 文本上的**固有度量差**（AOT 用真字体更准，非逻辑 bug；CLI 无字体无法精确匹配）。**结论**：20 批布局算术全部 CLI≡AOT 干净（整数确定性纪律已保证）；case-050 测量差属已知限制类别（非本周期引入，任何 CJK 文本均存在）。看门狗诊断代码已移除（@ce1ea43b，CLI 1345 零副作用）。**剩余收口项**：reactive-bench 双跑汇总报告（需重编 rb exe） |
...
### 28. 目标模式续：038 子约束链全清 + 三项否定归因（本轮）

**成果链**：216→196（case-038 20→0 全清 @21e7f1ba，零回归，31/32）。
- **038 治本**：buildChildSpace 对 auto 宽父直传祖先宽——父被 margin 收窄后子仍携宽约束（ul margin-left:24 → li 预布局 716 vs 应 692）。约束层扣父自身 margins（§10.3.3，Blink"先定自身再造子约束"的等价），正是此前被否定的 stack 时矫正（255→431）的正确对偶。**教训：预布局约束错配类问题的治本点在约束构造层，不在 stack 消费层**。
- **IFC atomic 平移 @639f6f06**（前 turn）：平移完备性不变量第五处（054 15→12）。

**三项否定归因（全部回退零残留，入储备）**：
1. **044/055 classic 滚动条 15px**：约束扣宽实验 055 -2 生效但 L05/L06 门捕——滚动条占位是环境数据要素（Edge classic vs 引擎 overlay），需连带 L 基线整体定性的专批。
2. **054 inline-block 含块子竖排**：selectAlgorithm blockAlgo 路由插桩实锤 HIT 但内部仍不竖排（内部排列在另一 inline 路径），且回退后反而 -1——结构性改造储备。
...
### 30. case-054 inline-block BFC 竖排治本（2026-07-29）

**批次**：inline-block 含块子竖排批（全量 180→171，-9；054 12→3；31/32 gates）

**归因（储备兑现）**：
- px-9（inline-block w100）含两个 block 子，B 竖排（h52、同 x）vs E 单行横排（h28、第二块 x=196）。
- 前轮 selectAlgorithm 路由 blockAlgo 实验"HIT 但不竖排"被否定——内部排列真身在 `InlineAlgorithm::layout` 的单行排列循环（与 §29 同一函数，横排 `$cursorX +=` 恒适用于一切子）。
- 治本（CSS 2.2 §9.2.2/§9.4.1 inline-block 是 BFC 根）：窄口径 `$allBlockKids`（全部子 display ∈ block/flex/grid/table）时走竖排堆叠：
  - 兄弟 margin 折叠 `max(mb,mt)`（§8.3.1），BFC 根阻断穿透折叠、首子 margin-top 保留在内；
  - 子放置用 `translateFragmentTree` 整树平移（平移完备性不变量——首版 new PhysicalFragment 顶层搬运致孙辈滞留，elem[6] y=40 残差实锤，改整树后消除）；
  - auto 高 = 竖排累计 + 末子 margin-bottom + 自身 padding/border 纵边缘（横排分支的 max 子高模型不适用，条件加 `!$allBlockKids`）。

**结果**：054 12→3（GEOMETRY 10→3，MISMATCH 2→0；height 52→76 对齐）；全量 180→171 零 case 回归；31/32。
余 3：elem[8] 根高固有族（B 锚点 y=750 vs E 116，页面级高度背景差）+ 两 span y 8px 基线精度（inline-block 末行基线模型）。

**方法论**：§29 与本批连环实锤"同函数多分支"论——046 宽路径与 054 排列路径同居 `InlineAlgorithm::layout`，一批修通后趁热复查同函数其余分支收益显著（两批合计 -25，全部同函数）。

**剩余榜（171）**：048(26)/050(25) 精度+固有差；044+055(24) classic 滚动条族；051(10) y 链；019(9)/007(8)/015(8)/021(8)/045(8)/047(8)/046(7)/053(7) 精度长尾；054(3)/043(4)/014(4) 尾差。

### 31. case-051 aspectRatio 烘焙串旁路治本（2026-07-29）

**批次**：aspect-ratio 消费点解析批（全量 171→162，-9；051 10→1；31/32 gates）

**归因（§29 方法论直接复用——"解析已修"≠"全路径已修"）**：
- @0845fbe6 已把 CssMappings 的 aspect-ratio parser 指向 parseAspectRatio，但 051 仍余 y 链 10 项（elem[1] E h=12 vs B 112.5，下游 y 全 +101）。
- h=12 = 200/16 反演 → ratio 仍以 16 进入。gen 产物实锤：sfc-compiler 把内联 style 烘焙为原始串 `'aspectRatio'=>'16/9'` 直塞 VNode style，**不经 StyleResolver parser 链**，ComputedStyle 消费点 `(float)'16/9'`=16。
- 治本：ComputedStyle `aspectRatio` 消费点对字符串值一律走 `CssValueParser::parseAspectRatio`（CSS-Sizing-4 §5 a/b 语法），CssLength/数值路径不变。同时覆盖运行时动态 style 同类旁路。

**结果**：051 10→1（余 1 = br 锚点根高固有族，与 054 elem[8] 同族：B 页面级根高背景差）；全量 171→162 零 case 回归；31/32。
...
### 33. br 字体行盒横扫批（2026-07-29）

**批次**：br getBoundingClientRect 语义批（全量 157→138，-19 横扫；31/32 gates）

**归因（019 只读归因兑现）**：
- 019 elem[141]（br）E h=32（=行盒高 `$line->height()`）vs B h=24、y 差 6。
- Blink 语义：br 是非替换 inline，getBoundingClientRect 高 = **字体行盒**（B 真值 @fs16 asc19/desc5 → h=24、top=基线−19），非其所在行盒高；E 直接给行高且 top=行顶。
- 治本（InlineAlgorithm 放置段 FORCED_BREAK 分支）：`asc=intdiv(fs*19+8,16)`、`desc=intdiv(fs*5+8,16)`（纯整数，asc19/desc5 真值系比例），`y = 行顶 + max(0, line->baseline − asc)`、`h = asc+desc`。

**结果（横扫 5 case 零劣化）**：021 line-height 8→0 **全清**、035 text-indent 2→0 **全清**、012 position-relative 1→0 **全清**、015 8→2、019 9→7；全量 157→138。
- 021/035/012 尾差全是 br 几何——br 恒以行高盒导出污染多 case 的 y/h 族，一处治本连锁清零。

**剩余榜（138）**：050(25) 精度+固有差；048(21) 色注记+精度（再构造链储备）；044+055(24) classic 滚动条族；007/045/047(各8)、046/053(各7)、019(7 右对齐 x±2 精度) 长尾；043/014(4)、054(3)、022/002/005(2)、011/026/051/052(1) 尾差。14+3=17 case 完全清零。

### 34. case-043 textarea 采集口径同构（2026-07-29）

**批次**：导出口径批（全量 138→134；043 4→0 **全清**，通过数 34→35/55；31/32 gates）

**归因**：
- 043 剩 4 项全为 STRUCTURE：3 个 engine-only textarea + 1 count diff。
- 口径实锤：全库 55 case 的 browser_ref 中 textarea **恒 0 个**（Chrome 采集脚本从不导出 textarea），E 导 3 个。前批已跳过 replaced 控件后代（@5a1b4dc2），本体未同步。
- 修复：LayoutNormalizer flatten 对 textarea 本体直接 return（与 #text/testroot 同级的口径豁免）；input/select 本体保留（B 侧有对应采集）。

**结果**：043 4→0 全清（第 18 个清零 case），全量 138→134 零回归，31/32。

**剩余榜（134，累计 4500→134 = -97.0%，18 case 清零）**：050(25)/048(21) 精度+固有差+再构造链储备；044+055(24) classic 滚动条族；007/045/047(各8)、046/053(各7)、019(7 右对齐 x±2) 长尾；014(4 导出注记：overflow/margin-auto used 值，归因完毕待修）；054(3)、022/002/005(2)、011/026/051/052(1) 尾差。

### 35. overflow 简写展开横扫批（2026-07-29）

**批次**：014 归因兑现 → 导出注记横扫（全量 134→114，-20；31/32 gates）

**归因**：
- 014 elem[68]（.oh-container）E overflow-x/y=visible vs B hidden，而 E `overflow=hidden` 简写在——引擎 overflowX/Y 通道未从简写展开恒 visible（CSS-Overflow-3 §3.1 简写同时置两轴）。
- 修复（LayoutNormalizer 导出规范化）：overflow 简写非 visible 且轴值为缺省 visible 时以简写覆盖（显式轴声明优先）。

**结果（横扫 5 case）**：047 object-fit 8→0 **全清**、015 min-max-height 2→0 **全清**、022 white-space 2→0 **全清**、044 background-attachment 12→6、014 4→2；全量 134→114 零回归；31/32。22 case 完全清零。
...
### 37. margin:auto used 值几何推断否定（2026-07-29）

**实验与否定**：007/014 余项均为 margin:auto used 值注记（B 报 used px vs E 0px）。Normalizer 层几何推断（block + 导出 margin 0 + 父 content 区两侧等隙>0 → used=隙）实测**误报**：007 elem[0]/[16]/[109] 等"恰好等隙但非 auto"块被错标（8→8 且换族），已回退零残留。

**定性升级**：纯几何无法区分"margin:auto 居中"与"其它机制致等隙"（父窄内容、恰好对称等）。治本必须在**引擎导出层**：布局消费 auto 时把 used 值（或 auto 哨兵）写回 dump style（`marginLeftAuto` 哨兵已有消费先例见 LayoutValidationStep），Normalizer 直读。属引擎 dump 序列化改造，储备。

**剩余榜（110 不变）**：050(25)/048(17)；055(12)+044(6)；045(8)、007(8 margin-auto 注记)、046/053(各7)、019(7)；054(3)、002/005/014(各2)、011/026/051/052(各1)。

### 38. margin:auto 哨兵序列化批（2026-07-29）

**批次**：引擎 dump 序列化批（全量 110→104，-6；007 8→2；31/32 gates）

**治本（§37 否定后的正道兑现）**：
- **序列化层**（Application::fragmentStyleToArray）：CssRect margin 分支在 `toPx()`（auto→0）之外补导 `marginLeftAuto/marginRightAuto` 哨兵（`CssLength::isAuto()` 判别）——used 值信息在此前的导出中被不可逆抹平。
- **消费层**（LayoutNormalizer::flatten）：以哨兵为门禁 + 父 content 区 rect 反推 used margin（getComputedStyle 报 used px，CSS 2.2 §10.3.3）；未导出的引擎匿名 wrapper 穿透传上游 content 区（不更新 CB 基准）。
- 与 §37 被否定的纯几何等隙推断的本质区别：**哨兵门禁使推断只作用于真 auto 声明**，恰好居中的非 auto 块零触碰。

**结果**：007 8→2（-6，B 225 族对齐），全量 110→104 零回归，31/32。
余差定性：007 elem[70] E used 250 vs B 225、014 elem[68] E 275 vs B 250——两侧各自几何自洽（used=各自 CB 与盒宽的真实解），差源于**父链 CB 宽度本身的 25/50px 差**（几何族，非注记），归入对应 case 的几何储备。

**剩余榜（104，累计 4500→104 = -97.7%）**：050(25)/048(17) 精度+固有差；055(12)+044(6) 滚动条族；045(8)、046/053(各7)、019(7) 长尾；054(3)、002/005/007/014(各2)、011/026/051/052(各1) 尾差。

### 39. text-decoration 族采集口径同构（2026-07-29）

**批次**：口径同构批（全量 104→95，-9，**跌破 100**；31/32 gates）

**归因**：
- 045 全部 8 项为 `text-decoration-line/color/style/thickness` engine-only MISMATCH（"browser has no value"）。
- 口径实锤：全库 55 case 的 browser_ref 中 text-decoration-* 键**恒 0 个**（dump_layout.js 不采集该族），E 无条件导出 → 恒无对照物。与 §34 textarea 同款口径豁免。
- 修复：LayoutNormalizer 导出末端统一剔除 text-decoration 族五键。
...
### 42. 050 `<q>` UA 自动引号批（2026-07-29）

**批次**：q open/close-quote 生成批（全量 84→72，-12；050 25→13；31/32 gates）

**归因（三轮迭代收敛）**：
- 前两轮误判（"CJK 半宽精度"→"未定位结构 2×"）后逐 span dump 实锤：px-222/px-227 两个 `<q>` E w=56/24 vs B 112/48，内部 span 渐进右移 +32/+56（跳变 = 外层 close + 内层 open 引号插入）——Chrome UA `q::before{content:open-quote}`/`::after{content:close-quote}`（Blink html.css，CSS2 §12.3.2），E 未生成。
- B 真值精确反演 @fs16：**open 恒 16=1em（含嵌套）**；**close 一级 16、嵌套二级 8=0.5em**（quotes 层级：二级单引号 glyph 窄）。

**治本（InlineAlgorithm 双点）**：
1. `buildInlineItems`：q 元素 OPEN/CLOSE_TAG 边缘宽加引号 glyph 宽（openQW=1em 恒定；closeQW 按 `$qDepth`（新签名参数，q 嵌套深度）一级 1em/二级 0.5em）；
2. 盒栈 union 重建：q 盒 rect 双侧各自从 open/close item 宽反推引号外扩（与 ① 同源，免二次分级）。

**结果**：050 q 族 12 项全清（25→13，余 13 = y=5px text-emphasis 注解行高族，§y1 储备：简写未展开致行高增补不触发）；全量 84→72 零 case 回归；31/32。

**方法论**：三轮归因纠偏链（精度→结构→特性缺口）证明"大值恰 2×"不可靠猜测——必须 dump 到 pxId 级并做 B 真值整数反演（跳变点 +24 = 相邻引号对）才能定性。

**剩余榜（72，累计 4500→72 = -98.4%）**：048(17) table 几何精度；050(13) 注解行高储备；055(12)+044(6) 滚动条族；019(7 CB 宽度精度)；054(3)、002/005/007/014/046(各2)、011/051/052/053(各1) 尾差。

### 43. 050 emphasis 注解 half-leading 半分批（2026-07-29）

**批次**：text-emphasis 注解占位分配模型批（全量 72→60；050 13→1；31/32 gates）

**归因（分叉点定位器实锤）**：
- 按文档序 dY 变迁扫描（锚点归一）：分叉在 te-circle 内首 span（px-80/px-118）——E 子相对容器顶 +10（emphExtra 全额压 over 侧）vs B +5，容器高相同（35）。
- B 模型：emphasis 增高 10px 按 **half-leading 上下对半分**（over 5 + under 5），文本下移仅半额——与 CSS 行盒 half-leading 一致（增高对称分布），非"全额让出 over 注解带"。

**治本（一行）**：IFC atomic 放置 `$atomY` 的下移量 `$emphExtra` → `intdiv($emphExtra+1, 2)`（round-half-up）；行推进 `$cursorY += line.height + $emphExtra` 保持全额（容器总高不变，B h=35 同值背书）。
...
### 45. 055 滚动条占宽 + flex auto-fill 扣 margin 双根因批（2026-07-29）

**批次**：滚动条族第一击（全量 60→49，-11；055 12→1；31/32 gates 快照更新后）

**双根因（B 真值 733/717 双层反演）**：
1. **经典竖向滚动条占宽**（buildChildSpace）：父 overflow-y ∈ {scroll,auto} 且显式非百分比高（定高溢出场景）→ 子约束宽扣 15（Windows classic 15px，CSS-Overflow-3 §3.3 scrollbar gutter；055 B 容器 content 733=748−15 实锤）。历史 §am1 否定实为"收益-2 vs 动基线"权衡弃案，非机制错误——本轮 055 主场收益 -11 兑现。
2. **flex 容器 auto-fill 漏扣自身 margins**（FlexAlgorithm L128 `$w=$parentW` 直填）：与 §28 Block 子约束链同源缺口的 Flex 容器版（055 px-10 margin:8 → E 733 vs B 717=733−16 实锤）→ `max(0, parentW − ml − mr)`。

**门捕处置**：L05 快照 4 行 400→385（-15）= 定高滚动容器子宽合法语义变更（功能测试 13/14→13/13 全过），照 §41 先例 `--update-snapshots` 更新，31/32 恢复。

**结果**：055 12→1（余 1 = br 锚点根高固有族）；全量 60→49 逐 case 零回归（044 的 6 为另形态待查）；31/32。

**剩余榜（49，累计 4500→49 = -98.9%）**：048(17) table 几何；019(7 CB 精度)；044(6 滚动族另形态)；054(3)、002/005/007/014/046(各2)、011/050/051/052/053/055(各1) 尾差。
