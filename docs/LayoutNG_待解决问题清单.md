...
| **6.7** | ~~全量 vs 单跑结果不一致~~ ✅ **已关闭 @975fa544**：探针证明 engine dump 两模式逐字节一致（确定性无缺陷）；差异全在浏览器侧——批量页 CSS scope 化平面正则非容错，40/55 case 资产 CSS 花括号不平衡（* {/body { 未闭合，98 处）使规则边界错乱：后续规则未 scope 泄漏跨 case 污染 + case 自身规则失效；单页模式浏览器靠错误恢复（CSS Syntax §5.4.4）存活规则——两模式测的是不同真值。资产补闭合后全量≡单跑恢复，总 diff 14814→10178 (-31%) | 会话 2026-07-26 双跑实证→根治 |
...
| 状态 | 数量 | 编号 |
|------|------|------|
| ❌ 未实现 | 6 | 1.1, 1.2, 1.4, 2.1, 2.4(仅 5px 半边), 2.5 |
| ⚠️ 部分实现 | 14 | 1.3, 2.2, 2.3, 2.6, 2.7, 2.8, 2.10, 2.12, 2.13, 4.1, 5.3, 5.4, 5.5, 5.6 |
| ✅ 已关闭 | 4 | 2.9（deb1ea36）、2.11（3f56927d，四次核验误判）、**5.1+5.2（18769d38 T1 批次：syncBindValues/isScrollContainer 路由 ScrollManager、PP fallback 收敛、死常量删除、expandAll 默认值收敛）** |
...
| ~~P0~~ | ~~5.2~~ | ~~破损代码清理（scroll bind 失效）~~ | ✅ **已关闭 @18769d38**（五次核验修正描述：主路径 L745/L1056 早已路由 ScrollManager，破损仅剩 syncBindValues 跳过路径 + L963 + PP fallback，非"完全失效"；三处已收敛单通道） |
