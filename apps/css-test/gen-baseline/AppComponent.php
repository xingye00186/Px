...
class AppComponent extends ReactiveComponent
{
    ...
    {
        // 静态 VNode 跨帧复用
        self::$__s0 ??= VNode::h('div', ['style'=>'...;font-size:18px;font-weight:700;color:#cdd6f4;'], 'Test Console');
        return VNode::h('#root', ['title'=>'Untitled','style'=>'...;width:1600px;height:800px'], VNode::h('div', ['class'=>'test-console','style'=>'...;width:1600px;height:800px;display:flex;font-family:\'Segoe UI\',\'Noto Sans SC\',system-ui,-apple-system,sans-serif;'], [
                            VNode::h('div', ['class'=>'sidebar','style'=>'...;width:280px;height:100%;background:#1e1e2e;display:flex;flex-direction:column;flex-shrink:0;'], [
                                    VNode::h('div', ['class'=>'sidebar-header','style'=>'...;padding:16px 20px;background:#181825;border-bottom:1px solid #313244;flex-shrink:0;'], [
                                            self::$__s0,
                                            (VNode::h('div', ['style'=>'...;font-size:11px;color:#6c7086;margin-top:2px;'], 'CSS Test Suite · ' . $this->caseCount . ' cases'))->withPatchFlags(32),
                                        ]),
                                    VNode::h('div', ['class'=>'case-list','style'=>'...;flex:1;overflow-y:auto;padding:6px 0;min-height:0;'], $this->render_0()),
                                    VNode::h('div', ['class'=>'sidebar-footer','style'=>'...;padding:10px 16px;border-top:1px solid #313244;flex-shrink:0;'], (VNode::h('button', ['style'=>'...;width:100%;height:32px;background:#313244;border:none;border-radius:6px;color:#cdd6f4;font-size:12px;cursor:pointer;','@click'=>'refreshCases'], 'Refresh List'))->withPatchFlags(4)),
                                ]),
                            VNode::h('div', ['class'=>'main-content','style'=>'...;flex:1;display:flex;flex-direction:column;background:#f5f5f5;'], [
                                    ...
                                    VNode::h('div', ['class'=>'content-body','style'=>'...;flex:1;padding:20px;overflow:auto;min-height:0;'], (VNode::hComponent($this->resolveComponent($this->caseName), ['style'=>'...'], []))->withPatchFlags(16)),
                                ]),
                        ]));
    }
    ...
    {
        switch ($bindKey) {
            case 'caseCount': if ($this->caseCount !== $value) { $this->caseCount = $value; } break;
            case 'currentTitle': if ($this->currentTitle !== $value) { $this->currentTitle = $value; } break;
            default: break;
        }
    }
    ...
    private function render_0(): array
    {
        $children = [];
        foreach ($this->caseList as $item) {
        $__v = VNode::hKey('div', ['style'=>'...;display:flex;align-items:center;height:34px;padding:0 20px;font-size:13px;color:#cdd6f4;cursor:pointer;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;','@click'=>'selectCase','click-arg'=>$item['tag'],':style'=>['background'=>$item['bgColor']]], [
                    $item['title']
                ], $item['tag']);
        $__v->patchFlags = 5;
        $children[] = $__v;
