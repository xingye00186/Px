...
void php_sk_begin_frame() {
    ...
    if (g_skCanvas) {
        g_skCanvas->save();
        // 每帧开头 clear 画布（避免残留上一帧）+ 替代 sk_clear_window 退化为空
        g_skCanvas->clear(SK_ColorWHITE);
    }
#endif
}
...
void php_sk_fill_rect(Int x, Int y, Int w, Int h, Int rgb) {
    ...
    // 1px 分隔线/边框在 setAntiAlias(true) 下会模糊扩散到 ~3px
...
// PHP 传入 X Y 为 text-top 坐标，Skia drawString 需 baseline 坐标，内部用 font metrics 转换
