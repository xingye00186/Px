...
| 修改 framework 公共 API（Css/Layout/Paint 等被 AOT 编译目录） | [lessons.md](docs/agents/lessons.md) + [coding-conventions.md](docs/agents/coding-conventions.md) | 不得混入编译期逻辑（L33）+ 背景色/box-sizing 语义（L34/L35） |
...
| [aot-constraints.md](docs/agents/aot-constraints.md) | 编写/修改 AOT 兼容代码时 | 禁止模式 + C2440/C2446 修复模式 + 闭包限制 + native_types 陷阱 + 公共 API 架构边界 |
...
| [lessons.md](docs/agents/lessons.md) | 修复 bug / 排查问题时先检索 | 踩坑案例库：症状→根因→修复→规则（AOT/渲染/布局） |
| [coding-conventions.md](docs/agents/coding-conventions.md) | 修改框架代码前必读 | 36 条检查清单（含 AOT 架构规则）+ PHP/VNode 编码规范 |
